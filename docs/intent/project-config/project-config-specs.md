...
- [x] **CONF-001**: The repository shall provide `project.toml.example` with a `[config]` table (`secrets_mode` defaulting to `"project_toml"`, `embedding_similarity_threshold` defaulting to `0.85`) and a `[secrets]` table (`openrouter_api_key`, `litellm_master_key`, `postgres_password`, `tailscale_auth_key`, `bws_access_token`).
...
- [x] **CONF-004**: `render_config.py` shall write `infra/generated.auto.tfvars.json` containing `tailscale_auth_key`, `bws_access_token`, and `secrets_mode`, read directly from `project.toml` with no branching logic.
...
- `docs/intent/aws-infra/aws-infra-specs.md` — consumes `CONF-004`/`CONF-009` for `[secrets].tailscale_auth_key`/`bws_access_token` and `[config].secrets_mode`; owns the Bitwarden pre-clone bootstrap this leaf deliberately doesn't integrate with
