...
- [x] **CONF-001**: The repository shall provide `project.toml.example` with a `[secrets]` table (`openrouter_api_key`, `litellm_master_key`, `postgres_password`, `tailscale_auth_key`).
...
- [x] **CONF-003**: `render_config.py` shall write `.env` containing `OPENROUTER_API_KEY`, `LITELLM_MASTER_KEY`, and `POSTGRES_PASSWORD`.
...
- `docs/intent/local-launch/local-launch-specs.md` — consumes `CONF-009` for `[secrets].openrouter_api_key`/`litellm_master_key`/`postgres_password`
