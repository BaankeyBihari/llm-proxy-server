...
- [x] **IGNITE-001**: Where a `size` query parameter is provided, the ignition function shall accept only `t4g.small`, `t4g.medium`, `t3.small`, or `t3.medium` as valid values.
...
- [x] **IGNITE-003**: When no `size` query parameter is provided, the ignition function shall default to `t4g.small`.
