...
`scripts/launch.sh` is a single entrypoint shared with the `aws-infra` leaf, dispatched by `--env=local`/`--env=aws` — the two paths differ only in owned keys and the trailing bring-up step (`docker compose up -d` here vs. `terraform apply` there); this leaf owns the `--env=local` path and everything below.
