...
4. For each owned `key = "value"` line, show the current value and prompt for a replacement. No mode-awareness here — this path always prompts unconditionally, even against a Bitwarden-provisioned host's `project.toml` (see `project-config-design.md`'s `secrets_mode` decision for why that's accepted, not guarded against).
