...
3. Read `project.toml` line by line, in file order, via the shared loop in `scripts/lib/project-toml.sh` (`project-config`'s territory) — scoped to this path's keys: `[secrets].openrouter_api_key`/`litellm_master_key`/`postgres_password`. `[table]` header lines pass through unchanged (tracked for prompt display, e.g. `secrets.openrouter_api_key`); blank lines and `#`-comment lines pass through unchanged.
4. For each owned `key = "value"` line, show the current value and prompt for a replacement. No mode-awareness here — this path always prompts unconditionally, even against a `project.toml` a `bws-sync.sh` run just populated (see `aws-infra-design.md`'s "Bitwarden Sync Script" section).
