...
`secrets_mode` (`"project_toml"` default — shared across all deploy targets now, see `project-config-design.md` for why `"bitwarden"` can no longer be the shipped default — or `"bitwarden"`, opted into via `launch.sh --env=aws`; renamed from `"env_file"`) picks whether `user_data` fetches `OPENROUTER_API_KEY`/`LITELLM_MASTER_KEY`/`POSTGRES_PASSWORD` from Bitwarden at boot. See `docs/gemini/bitwarden.md` for the Bitwarden Secrets Manager setup this assumes.

- **`bitwarden`**: `user_data` installs the ARM64 `bws` CLI and runs `bws secret list --output env`, authenticated with the `bws_access_token` variable (`sensitive`, empty default), writing the result to `/home/ubuntu/.env`. This runs once, at first boot, **before the repo is cloned**, and is the *only* place `bws` is ever invoked on this target — `render_config.py` never branches on `secrets_mode` and never shells out to `bws` itself (`project-config-design.md`'s explicit decision: keeping `bws_access_token` out of the host's own `project.toml` matters more than making a later `launch.sh --env=local` re-run Bitwarden-coherent). The one remaining manual step (repo clone, below) picks the pre-clone file up with `cp ~/.env ~/litellm-proxy/.env`. If an operator later runs `launch.sh --env=local` on this host anyway (deviating from the documented flow, which stops at `cp`), it will prompt through `project.toml` and overwrite that `.env` with whatever's typed there — accepted, not guarded against; see `project-config-design.md`'s decision table for why.
- **`project_toml`**: `user_data` does nothing extra. The operator runs `./scripts/launch.sh --env=local` over the same Session Manager session used for the repo clone, exactly as before this variable existed.

`scripts/launch.sh --env=aws` (see below) is what actually sets `secrets_mode` and `bws_access_token` at `apply` time — an operator doesn't hand-edit `main.tf` or type `-var` flags per run.

## Bitwarden Vault Check Script

`scripts/bws-secrets-check.sh` runs on the operator's laptop, independent of `terraform apply` — reviewing/updating what's actually stored in the Bitwarden Secrets Manager project is a vault-maintenance task, not a deploy step, so it isn't wired into `launch.sh --env=aws`.
...
`BWS_ACCESS_TOKEN` (this script's env var) and `infra/terraform.tfvars`' `bws_access_token` (what `launch.sh --env=aws` writes) are the same token value held in two separate places — setting one does not set the other; an operator switching between the two scripts re-supplies it each time.
...
The `--env=aws` path uses `project-config`'s shared prompt loop (`scripts/lib/project-toml.sh`), pointed at `project.toml`'s `[tailscale]` and `[aws]` tables (`tailscale_auth_key`, `secrets_mode`, `bws_access_token`) instead of its own copy of the loop against `infra/terraform.tfvars` — see `project-config-design.md` for why the two paths share one implementation instead of two. It then calls `scripts/render_config.py` to produce `infra/generated.auto.tfvars.json`, and runs `terraform -chdir=infra init` and `terraform -chdir=infra apply` — no `-var-file` flag; Terraform auto-loads `*.auto.tfvars.json` natively.

Before the prompt loop runs, `--env=aws` also checks `$BWS_ACCESS_TOKEN` (the `bws` CLI's own env var, same one `bws-secrets-check.sh` reads) and pre-fills `project.toml`'s `bws_access_token` with it — but only when that field is still the untouched `""` placeholder; an already-configured real value is never silently overwritten. This is local to `aws_launch()`, not the shared loop itself — `project-toml.sh` stays key-agnostic, no per-key special-casing (`project-config-design.md`'s "no mode-awareness in the loop" tenet).
...
| `secrets_mode` variable validation | `validation` block restricting the variable to `["bitwarden", "project_toml"]` | Leave it a free-form string, fail only inside `user_data`'s bash `if` at boot time | A typo'd value should fail fast at `terraform plan`/`apply`, not silently skip both branches on a running instance discovered later. |
...
| `bws_access_token` pre-fill from `$BWS_ACCESS_TOKEN` | Only when `project.toml`'s field is still the placeholder `""`; `awk` rewrite local to `aws_launch()` | Always overwrite from the env var; add env-var awareness to the shared `project-toml.sh` loop itself | Placeholder-only avoids silently clobbering a real token an operator already saved in `project.toml`. Keeping it local to `aws_launch()` (not the shared loop) matches the loop's existing no-per-key-special-casing tenet; `awk` is already a dependency this repo uses (`aws-idle-check.sh`), so no new tool. |
...
4. **Accepted, not fixed**: `secrets_mode = "bitwarden"`'s `user_data` fetch to `/home/ubuntu/.env` predates `project.toml`'s existence on the host and stays that way — `render_config.py` never becomes Bitwarden-aware (see `project-config-design.md`'s `secrets_mode` decision row: keeping `bws_access_token` off the running instance outweighs making a later `launch.sh --env=local` re-run coherent with it). An operator who runs `launch.sh --env=local` on a Bitwarden-provisioned host anyway will overwrite that `.env` with `project.toml`'s values — the documented workflow never routes them there (`cp ~/.env` is the documented step), so this is a deviation the operator chooses, not a silent trap.
