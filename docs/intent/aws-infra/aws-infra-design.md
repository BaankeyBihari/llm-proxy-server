...
`secrets_mode` (`"project_toml"` default — shared across all deploy targets now, see `project-config-design.md` for why `"bitwarden"` can no longer be the shipped default — or `"bitwarden"`, opted into via `aws-launch.sh`; renamed from `"env_file"`) picks whether `user_data` fetches `OPENROUTER_API_KEY`/`LITELLM_MASTER_KEY`/`POSTGRES_PASSWORD` from Bitwarden at boot. See `docs/gemini/bitwarden.md` for the Bitwarden Secrets Manager setup this assumes.

- **`bitwarden`**: `user_data` installs the ARM64 `bws` CLI and runs `bws secret list --output env`, authenticated with the `bws_access_token` variable (`sensitive`, empty default), writing the result to `/home/ubuntu/.env`. This runs once, at first boot, **before the repo is cloned**, and is the *only* place `bws` is ever invoked on this target — `render_config.py` never branches on `secrets_mode` and never shells out to `bws` itself (`project-config-design.md`'s explicit decision: keeping `bws_access_token` out of the host's own `project.toml` matters more than making a later `local-launch.sh` re-run Bitwarden-coherent). The one remaining manual step (repo clone, below) picks the pre-clone file up with `cp ~/.env ~/litellm-proxy/.env`. If an operator later runs `local-launch.sh` on this host anyway (deviating from the documented flow, which stops at `cp`), it will prompt through `project.toml` and overwrite that `.env` with whatever's typed there — accepted, not guarded against; see `project-config-design.md`'s decision table for why.
- **`project_toml`**: `user_data` does nothing extra. The operator runs `./scripts/local-launch.sh` over the same Session Manager session used for the repo clone, exactly as before this variable existed.
...
`aws-launch.sh` uses `project-config`'s shared prompt loop (`scripts/lib/project-toml.sh`), pointed at `project.toml`'s `[tailscale]` and `[aws]` tables (`tailscale_auth_key`, `secrets_mode`, `bws_access_token`) instead of its own copy of the loop against `infra/terraform.tfvars` — see `project-config-design.md` for why the two scripts now share one implementation instead of two. It then calls `scripts/render_config.py` to produce `infra/generated.auto.tfvars.json`, and runs `terraform -chdir=infra init` and `terraform -chdir=infra apply` — no `-var-file` flag; Terraform auto-loads `*.auto.tfvars.json` natively.
...
| Terraform variable prompting | `aws-launch.sh` sources `project-config`'s shared `scripts/lib/project-toml.sh` loop, scoped to `project.toml`'s `[tailscale]`/`[aws]` tables | A separate copy of the loop adapted for `.tfvars`' `key = "value"` syntax (this leaf's prior approach, when `.env` and `.tfvars` were different formats) | Superseded: once both `local-launch.sh` and `aws-launch.sh` edit the same file in the same syntax (`project.toml`), the format-divergence that justified two copies no longer exists — see `project-config-design.md`. |
...
- `docs/intent/project-config/project-config-design.md` — `project.toml` schema, shared prompt loop, and the generator `aws-launch.sh` calls for `infra/generated.auto.tfvars.json`
