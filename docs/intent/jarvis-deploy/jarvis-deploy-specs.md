...
- [x] **JARVIS-004**: If no `.env` file exists in the workspace, then the startup script shall create one populated with placeholder `OPENROUTER_API_KEY` and `LITELLM_MASTER_KEY` entries before starting the stack.
