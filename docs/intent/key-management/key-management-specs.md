...
- [x] **KEYS-007**: No boot script (`launch.sh --env=local`, `aws-start-stack.sh`, `jarvis-startup.sh.example`) shall invoke a database migration command; schema migration shall be handled automatically by LiteLLM's own startup when `DATABASE_URL` is set.
