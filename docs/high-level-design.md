...
| Local and AWS interactive setup share one script (`scripts/launch.sh --env=local|aws`) that populates `project.toml` per-key and then does the target's own bring-up (`docker compose up -d` for local, `terraform apply` for AWS), not a one-shot `cp .env.example .env` or two separate per-target scripts | Leave `cp .env.example .env` as the only documented step; keep separate `local-launch.sh`/`aws-launch.sh` scripts, one per target | A silent copy leaves placeholder secrets in place with no prompt to replace them; per-key confirmation surfaces that at setup time instead of at a failed OpenRouter auth call. Both targets already shared `project-toml.sh`'s prompt loop; only the trailing bring-up step and owned-key list differed, so one dispatched entrypoint is one fewer filename to remember without merging the `local-launch` and `aws-infra` leaves themselves. The running-container guard stays local-only — AWS's `terraform apply` has its own idempotency. |
